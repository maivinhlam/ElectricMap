...
export default function HomeScreen() {
  ...
  const [lastViewedImage, setLastViewedImage] =
  ...
  const loadLastViewedImage = async () => {
    try {
      const lastImageJson = await AsyncStorage.getItem("lastViewedImage");
      if (lastImageJson) {
        const lastImage = JSON.parse(lastImageJson);
        setLastViewedImage(lastImage);
      }
    } catch (error) {
      console.error("Failed to load last viewed image:", error);
    }
  };

  const saveLastViewedImage = async (image: MotorcycleImage) => {
    try {
      await AsyncStorage.setItem("lastViewedImage", JSON.stringify(image));
      setLastViewedImage(image);
    } catch (error) {
      console.error("Failed to save last viewed image:", error);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Last Viewed Image */}
      {lastViewedImage && !showResults && (
        <View style={styles.lastViewedSection}>
          <Text style={styles.sectionTitle}>Last Viewed</Text>
          <TouchableOpacity
            style={styles.lastViewedCard}
            onPress={() => handleImagePress(lastViewedImage)}
          >
            <Image
              source={{
                uri:
                  lastViewedImage.url || "https://via.placeholder.com/100x75",
              }}
              style={styles.lastViewedImage}
              defaultSource={{ uri: "https://via.placeholder.com/100x75" }}
            />
            <Text style={styles.lastViewedTitle}>{lastViewedImage.title}</Text>
          </TouchableOpacity>
        </View>
      )}
